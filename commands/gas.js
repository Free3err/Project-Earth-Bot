...
module.exports = async (bot, message, args, argsF) => {
    ...
    if (hasRole.roles.cache.has(role)) {
        try {
            ...
            const randomNum = Math.floor(Math.random() * (10000 - 5000) + 5000); // генерируем случайное число от 1 до 5
            const lastUsedTime = userData[userId].lastUsed_gas;
            ...
            message.reply({
                embeds: [{
                    title: 'Добыча газа',
                    description: 'Меню добычи газа',
                    fields: [{
                        name: 'Результат',
                        value: `Вы добыли ${randomNum} кубометров газа! Поздравляем!`
                    }],
                    color: 16528836
                }]
            });
            ...
            userData[userId].gas += randomNum; // добавляем количество руды в данные пользователя
            userData[userId].lastUsed_gas = currentTime;
