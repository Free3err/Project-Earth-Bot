...
module.exports = (bot) => {
    ...
    bot.on("messageCreate", async (message) => {
        ...
        if (commandRun) {
            ...
            const roleIdToCheck = '1098979765274292371';
            const member = message.guild.members.cache.get(message.author.id);
            
            if (member && member.roles.cache.has(roleIdToCheck)) {
                // Роль есть, выполняем команду
                commandRun(bot, message, args, argsF)
                    .catch(err => console.error(err));
            } else {
                // Роли нет, отправляем сообщение
                message.reply({
                    embeds: [{
                        title: "Отказано",
                        description: "Вы не являетесь страной-игроком. Пожалуйста, зарегистрируйтесь и получите роль <@&1098979765274292371>!",
                        color: 16528836
                    }]
                });
            }
